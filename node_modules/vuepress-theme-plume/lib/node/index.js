...
import { isEmptyObject as isEmptyObject2, isPlainObject as isPlainObject4 } from "@pengzhanbo/utils";
...
function detectDependencies(options, plugins) {
  ...
  if (markdown.artPlayer || mdPower.artPlayer)
    add("artPlayer");
  if (isEmptyObject2(shouldInstall))
    return;
  const features = Object.keys(shouldInstall);
  const dependencies = Object.values(shouldInstall).flat();
  logger.warn(`
Enabling features such as ${features.map((feat) => colors4.green(feat)).join(", ")} requires the installation of the following dependencies: ${dependencies.map((dep) => colors4.yellow(dep)).join(", ")}`);
  const agent = getUserAgent();
  if (agent) {
    const { command = "", args = [] } = resolveCommand(agent, "add", dependencies) || {};
    logger.info(`Run the command to install:
  ${colors4.cyan(`${command} ${args.join(" ").replace(DEPENDENCIES.twoslash[0], `${DEPENDENCIES.twoslash[0]}@next`)}`)}`);
  }
}
...
function detectMarkdown(options) {
  ...
  if (unsupported.length) {
    logger.warn(`
${colors5.green("markdown")} unsupported fields: ${unsupported.map((field) => colors5.yellow(`"${field}"`)).join(", ")}, please check your config.`);
  }
}
...
function detectPlugins(plugins) {
  ...
  if (unsupportedPluginsFields.length) {
    logger.warn(`
${colors6.green("plugins")} unsupported fields: ${unsupportedPluginsFields.map((field) => colors6.yellow(`"${field}"`)).join(", ")}, please check your config.`);
  }
}
